...
func main() {
	...
	if *downServers != "" {
		for _, downServer := range strings.Split(*downServers, ",") {
			...
			num, err := strconv.Atoi(downServer)
			if err != nil {
				log.Fatal("Down servers are not formated correctly.")
			}
			downServersList = append(downServersList, num)
		}
	} else {
	...
	hash_ring := surfstore.NewConsistentHashRing(numServers, downServersList)
	blockHashes := surfstore.ComputeHashList(inpFilename, blockSize)

	ret := hash_ring.OutputMap(blockHashes)
